<script lang="ts">
	...
	onMount(() => {
		...
		const gridSize = layerMode ? 12 : 28;
		...
		const monoliths = new THREE.InstancedMesh(monolithGeometry, monolithMaterial, instanceCount);
		monoliths.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
		monoliths.castShadow = false;
		monoliths.receiveShadow = false;
		scene.add(monoliths);

		const basePositions = new Float32Array(instanceCount * 3);
		const phaseOffsets = new Float32Array(instanceCount);
		const colorNoise = new Float32Array(instanceCount);
		const centerOffset = (gridSize - 1) * 0.5;
		...
		const setRendererPixelRatio = (tier: PerfTier) => {
			...
			let next = cap;
			if (tier === 'medium') {
				next = layerMode ? Math.max(0.5, cap * 0.88) : Math.max(1, cap * 0.88);
			}
			if (tier === 'low') {
				next = layerMode ? Math.max(0.5, cap * 0.74) : Math.max(1, cap * 0.74);
			}
