 ...
 */
export async function deleteTrack({ request }: { request: Request }) {
	try {
		const formData = await request.formData();
		...
		const success = await trackStore.delete(trackId);
 ...
 * Reorder tracks
 */
export async function reorderTracks({ request }: { request: Request }) {
	try {
		const formData = await request.formData();
		...
		const orderedIds = JSON.parse(orderData) as string[];
