<script lang="ts">
	...
	onMount(() => {
		...
		const setPixelRatio = (tier: PerfTier) => {
			...
			let ratio = cap;
			if (tier === 'medium') {
				ratio = Math.max(1, cap * 0.88);
			}
			if (tier === 'low') {
				ratio = Math.max(1, cap * 0.74);
			}
...
<svelte:head>
	<title>Three.js Four — Resonant Core Bloom</title>
	<meta
		name="description"
		content="Deforming geodesic core with luminous torus petals driven by frequency data"
	/>
</svelte:head>
...
<div class="relative min-h-screen overflow-hidden bg-black text-white">
	...
	>
		...
		>
			<div class="text-[11px] tracking-[0.2em] text-cyan-300/90 uppercase">Resonant Core Bloom</div>
